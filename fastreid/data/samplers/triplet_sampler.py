        ...
        while True:
            ...
            while len(avl_pids) >= self.num_pids_per_batch:
                ...
                if len(batch_indices) == self.batch_size:
                    yield from reorder_index(batch_indices, self._world_size)
                    batch_indices = []
class AGSampler(Sampler):
    """
    Randomly sample N identities, then for each identity,
    randomly sample K instances, therefore batch size is N*K.

    then for each instance,
    K/n from v1,v2...,vn view
    ...
    def __init__(self, data_source: str, mini_batch_size: int, num_instances: int, seed: Optional[int] = None):
        ...
        # self.pid_index = defaultdict(list)
        self.pid_index = defaultdict(lambda: defaultdict(list))
        ...
        for index, info in enumerate(data_source):
            ...
            view = info[3]
            view_set.add(view)
            self.pid_index[pid][view].append(index)

        self.view_num = len(view_set)

        assert self.num_instances % self.view_num == 0,  "num_instance应该是view的整数倍"
                    ...
                    if pid not in batch_idxs_dict:
                        ...
                        if len(idxs) < self.num_instances:
                            ...
                            for i in range(self.view_num):
                                idxs_new.extend(np.random.choice(idxs[i], size=self.num_instances // self.view_num, replace=True).tolist())
                            idxs = idxs_new
