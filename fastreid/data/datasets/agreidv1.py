...
def _get_agreid_image_paths(dir_path):
    image_dir = osp.join(dir_path, 'image')
    search_dir = image_dir if osp.isdir(image_dir) else dir_path
    return glob.glob(osp.join(search_dir, '*.jpg'))


def _resolve_data_dir(root, dataset_dir):
    root = osp.abspath(osp.expanduser(root))
    candidates = [osp.join(root, dataset_dir), osp.join(root, dataset_dir.lower()), root]
...
class AG_ReID_v1(ImageDataset):
    dataset_dir = "AG_ReID_v1"
    dataset_name = 'agreidv1'

    def __init__(self, root='datasets', verbose=True, **kwargs):
        self.view_dict = {0:0, 3:1} # 0:UAV,3:CCTV

        self.root = root
        self.dataset_dir = _resolve_data_dir(root, self.dataset_dir)
        ...
        super(AG_ReID_v1, self).__init__(train, query, gallery, **kwargs)
    ...
    def _process_dir(self, dir_path, is_train=True):
        img_paths = _get_agreid_image_paths(dir_path)
    ...
    def name_of_attribute(self):
        if not self.key_attribute:
            raise ValueError("Attribute names are not loaded.")
...
@DATASET_REGISTRY.register()
class AG_ReID_v1_GA(ImageDataset):
    dataset_dir = "AG_ReID_v1"
    dataset_name = 'agreidv1'
    ...
    def __init__(self, root='datasets', verbose=True, **kwargs):
        ...
        super(AG_ReID_v1_GA, self).__init__(train, query, gallery, **kwargs)
...
@DATASET_REGISTRY.register()
class AG_ReID_v1_AG(ImageDataset):
    dataset_dir = "AG_ReID_v1"
    dataset_name = 'agreidv1'
