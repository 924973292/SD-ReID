...
def _dataset_root(cfg):
    return getattr(cfg.DATASETS, "ROOT", _root) or _root
...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            data = DATASET_REGISTRY.get(d)(root=root, **kwargs)
    ...
    if sampler is None:
        ...
        elif sampler_name == "AGSampler":
            sampler = samplers.AGSampler(train_set.img_items, mini_batch_size, num_instance)
...
def _test_loader_from_config(cfg, *, dataset_name=None, test_set=None, num_query=0, transforms=None, **kwargs):
    ...
    if test_set is None:
        ...
        data = DATASET_REGISTRY.get(dataset_name)(root=_dataset_root(cfg), **kwargs)
        if comm.is_main_process():
            data.show_test()
