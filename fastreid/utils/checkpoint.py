...
class Checkpointer(object):
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> object:
        ...
        # # new_add
        # flag = 0
        # for k in incompatible[1]:
        #     if 'ID_features' in k:
        #         name = k.split('.')[-1]
        #         self.model.mm.ID_features[name] = nn.Parameter(torch.randn(11, 768))
        #         flag = 1
        #
        #     elif 'cls_features' in k:
        #         name = k.split('.')[-1]
        #         self.model.mm.cls_features[name] = nn.Parameter(torch.randn(768, 1, 1))
        #         flag = 1
        #
        # if flag == 1:
        #     checkpoint = self._load_file(path)
...
class PeriodicCheckpointer:
    ...
    def step(self, epoch: int, **kwargs: Any):
        ...
        if (epoch + 1) % self.period == 0 and epoch < self.max_epoch - 1:
            ...
            self.logger.info("the best_metric is {}".format(self.best_metric))
