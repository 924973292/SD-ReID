...
from fastreid.modeling.sd import (
    UNet,
...
class Baseline_stage2_old(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.SDMODEL.SCHEDULER_CONFIG.NAME == "euler":
            noise_scheduler = EulerDiscreteScheduler.from_pretrained(
                cfg.MODEL.SDMODEL.SCHEDULER_CONFIG.PRETRAINED_PATH
            )
        ...
        memory_bank = MemoryBank_base(
            cfg.MODEL.SDMODEL.VIEW_NUM, cfg.MODEL.SDMODEL.MOEMNTUM
        )
    ...
    def forward(self, batched_inputs):
        ...
        view_token_center, _, _ = self.mm(
            None, None, features, view, imgids, stage_condition=2
        )
        ...
        if self.training:
            ...
            if self.cubic_sampling:
                ...
                timesteps = torch.clamp(
                    timesteps, 0, self.noise_scheduler.config.num_train_timesteps - 1
                )
            else:
            ...
            c, down_block_additional_residuals = self.condition_learner(
                ID_features, view_token_new, view_token_center
            )

            encoder_hidden_states = c.to(dtype=weight_type)
            if down_block_additional_residuals is not None:
                down_block_additional_residuals = [
                    samper.to(dtype=weight_type)
                    for samper in down_block_additional_residuals
                ]
            ...
            view_token_new, _, _ = self.mm(
                None, None, None, view, imgids, stage_condition=2
            )
            ...
            if self.mm.view_num == 2:
                ...
                c1, down_block_additional_residuals1 = self.condition_learner(
                    ID_features, view_token_new1, view_token_new1
                )
                c2, down_block_additional_residuals2 = self.condition_learner(
                    ID_features, view_token_new2, view_token_new2
                )

                noisy_latents1 = torch.randn((bsz, 4, 16, 12)).to("cuda")
                noisy_latents2 = torch.randn((bsz, 4, 16, 12)).to("cuda")

                latents1 = self.inverse_sampling(
                    self.noise_scheduler,
                    weight_dtype,
                    c1,
                    noisy_latents1,
                    down_block_additional_residuals1,
                    self.inverse_noise_scheduler,
                    self.train_step,
                    self.switch2,
                ).flatten(1)
                latents2 = self.inverse_sampling(
                    self.noise_scheduler,
                    weight_dtype,
                    c2,
                    noisy_latents2,
