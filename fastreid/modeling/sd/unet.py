...
from diffusers.utils import USE_PEFT_BACKEND, BaseOutput, deprecate, is_torch_version, logging, scale_lora_layers, unscale_lora_layers
...
class UNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.model = ResidualUNet2DConditionModel.from_pretrained(
            cfg.MODEL.SDMODEL.UNET_CONFIG.PRETRAINED_PATH, use_safetensors = True)
        self.model.requires_grad_(False)
        try:
            self.model.enable_xformers_memory_efficient_attention()
        except ModuleNotFoundError:
            logger.warning("xformers is not available; falling back to standard attention.")
        ...
        for i, up_block in enumerate(self.model.up_blocks):
            if isinstance(up_block, ResidualCrossAttnUpBlock2D):
                for j, attn in enumerate(up_block.attentions):
                    ...
                    if block_idx not in cfg.MODEL.SDMODEL.UNET_CONFIG.TRAINABLE_BLOCK_IDX:
                        continue
                    ...
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_SELF_ATTN_Q:
                        self_attn.to_q.requires_grad_(True)
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_SELF_ATTN_K:
                        self_attn.to_k.requires_grad_(True)
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_SELF_ATTN_V:
                    ...
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_CROSS_ATTN_Q:
                        cross_attn.to_q.requires_grad_(True)
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_CROSS_ATTN_K:
                        cross_attn.to_k.requires_grad_(True)
                    if cfg.MODEL.SDMODEL.UNET_CONFIG.TRAIN_CROSS_ATTN_V:
    ...
    def forward(self, sample, timestep, **kwargs):
        ...
        output = self.model(sample, timestep, **kwargs)
