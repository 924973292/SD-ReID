...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int,
                 heads: int, output_dim: int, sie_xishu: float, camera_num: int):
        super().__init__()
        ...
        if sie_xishu != 0.0:
            self.cv_embed = nn.Parameter(torch.zeros(camera_num, width))
            trunc_normal_(self.cv_embed, std=.02)
            logger.info('camera number is : {}'.format(camera_num))
        else:
            self.cv_embed = None
            logger.info('cv_embed is None')
    ...
    def forward(self, x: torch.Tensor, camid):
        ...
        if self.cv_embed != None:
            cv_emb = self.sie_xishu * self.cv_embed[camid]
            x[:, 0] = x[:, 0] + cv_emb
        ...
        return xproj[:,0].unsqueeze(-1).unsqueeze(-1)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, camid):
        return self.visual(image.type(self.dtype), camid)
    ...
    def forward(self, image, text, camid):
        image_features = self.encode_image(image, camid)
...
def build_clip(cfg):
    # original input of clip build function:
    # state_dict: dict, h_resolution: int, w_resolution: int, stride_size: int

    # new in fastreid:
    # load the pretrained model from the path
    pretrain_path = cfg.MODEL.BACKBONE.PRETRAIN_PATH
    try:
        # loading JIT archive
        model = torch.jit.load(pretrain_path, map_location="cpu").eval()
    ...
    camera_num = cfg.MODEL.BACKBONE.CAMERA_NUM
    h_resolution = input_size[0]//stride_size[0]
    w_resolution = input_size[1]//stride_size[1]
    # fmt: on
    ...
    model = CLIP(
        embed_dim,
        image_resolution, vision_layers, vision_width, vision_patch_size, stride_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers,
        h_resolution, w_resolution, sie_xishu, camera_num
    )
    if vit:
        state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"],
                                                                     model.visual.positional_embedding, h_resolution,
