...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, embed_dim=768,
        ...
        self.view_prompt = nn.Parameter(torch.zeros(1, 1, embed_dim))
    ...
    def forward(self, x, camera_id=None):
        ...
        ID_conditions = []
        for i, blk in enumerate(self.blocks[:11]):
            x = blk(x)

            ID_conditions.append(x[:, 0])

        view_prompt = self.view_prompt.expand(B, -1, -1)
        x = torch.cat([view_prompt, x], dim=1)
        ...
        return x[:, 0].reshape(x.shape[0], -1, 1, 1), view_token.reshape(x.shape[0], -1, 1, 1), ID_conditions
