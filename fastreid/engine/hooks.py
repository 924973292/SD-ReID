...
class PeriodicCheckpointer(_PeriodicCheckpointer, HookBase):
    ...
    def before_train(self):
        ...
        if len(self.trainer.cfg.DATASETS.TESTS) == 1:
            ...
            self.metric_name = []
            for i in self.trainer.cfg.DATASETS.TESTS:
                self.metric_name.append(i + "/metric")
    ...
    def after_epoch(self):
        ...
        #     metric = metric + storage.latest()[i][0] if i in storage.latest() else -1
        # metric_dict = dict(
        #         metric=metric
        #     )
...
class EvalHook(HookBase):
    ...
    def after_epoch(self):
        ...
        #
        # # 节省时间，改成40epoch前不触发
        # if self._period > 0 and next_epoch % self._period == 0 and next_epoch > 35:
        #     self._do_eval()
