...
class MyTestCase(unittest.TestCase):
    def test_se_resnet101(self):
        pretrain_path = os.getenv('FASTREID_SE_RESNET101_IBN_A')
        if not pretrain_path:
            self.skipTest('set FASTREID_SE_RESNET101_IBN_A to run this pretrained-weight comparison')
        if not torch.cuda.is_available():
            self.skipTest('CUDA is required for this legacy comparison test')
        ...
        cfg.MODEL.BACKBONE.PRETRAIN_PATH = pretrain_path
